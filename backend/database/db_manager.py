...
def buscar_todos_os_itens(nome=None, categoria=None, oficial=None, criador_id=None):
    """Busca itens com filtros opcionais."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            conexao.row_factory = sqlite3.Row
            cursor = conexao.cursor()
            query = "SELECT * FROM itens_base WHERE 1=1"
            params = []
            if nome:
                query += " AND nome LIKE ?"
            ...
            query += " ORDER BY categoria, nome LIMIT 300"
...
def buscar_categorias_itens():
    """Retorna lista de categorias únicas para filtro."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            cursor = conexao.cursor()
            cursor.execute("SELECT DISTINCT categoria FROM itens_base WHERE categoria IS NOT NULL ORDER BY categoria")
...
def buscar_todos_os_monstros(nome=None, oficial=None, tipo=None, criador_id=None):
    """Busca monstros com filtros opcionais: nome, oficial (0/1), tipo."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            conexao.row_factory = sqlite3.Row
            cursor = conexao.cursor()
...
def buscar_monstro_por_id(monstro_id):
    ...
    try:
        ...
        print(f"Erro ao buscar monstro por id: {e}")
        return None

def buscar_tipos_monstros():
    """Retorna lista de tipos únicos de monstros para filtro."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            cursor = conexao.cursor()
            cursor.execute("SELECT DISTINCT tipo FROM monstros_base WHERE tipo IS NOT NULL ORDER BY tipo")
        ...
        with sqlite3.connect(NOME_DB) as conexao:
            ...
            cursor.execute(
                ...
                INSERT INTO monstros_base (
                    ...
                    cr, habilidades, fonte, oficial, criador_id
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    dados['nome'], dados.get('vida_maxima', dados.get('hp',1)),
                    dados.get('ataque_bonus',0), dados.get('dano_dado','1d6'),
                    dados.get('defesa', dados.get('ca',10)),
                    dados.get('xp_oferecido',0), dados.get('ouro_drop',0),
                    dados.get('tamanho','Medium'), dados.get('tipo','Unknown'),
                    dados.get('alinhamento','—'), dados.get('ca',10),
                    dados.get('deslocamento','30'),
                    dados.get('for_attr',10), dados.get('des_attr',10), dados.get('con_attr',10),
                    dados.get('intel_attr',10), dados.get('sab_attr',10), dados.get('car_attr',10),
                    dados.get('saving_throws',''), dados.get('skills',''),
                    dados.get('resistencias',''), dados.get('sentidos','Normal'),
                    dados.get('idiomas','—'), dados.get('cr','0'),
                    dados.get('habilidades',''), dados.get('fonte','Custom'), 0,
                    dados.get('criador_id', None)
                )
            )
        ...
        with sqlite3.connect(NOME_DB) as conexao:
            ...
            cursor.execute(
                ...
                INSERT INTO itens_base (nome, tipo, descricao, preco_ouro, dano_dado, bonus_ataque, efeito,
                    categoria, subcategoria, peso, fonte, oficial, criador_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    dados['nome'], dados.get('tipo', dados.get('categoria','Adventuring Gear')),
                    dados.get('descricao',''), dados.get('preco_ouro', 0),
                    dados.get('dano_dado','—'), dados.get('bonus_ataque', 0),
                    dados.get('efeito',''),
                    dados.get('categoria', dados.get('tipo','Adventuring Gear')),
                    dados.get('subcategoria','-'),
                    dados.get('peso', 0),
                    dados.get('fonte','Custom'),
                    dados.get('oficial', 0),
                    dados.get('criador_id', None)
                )
            )
...
def banir_usuario_sala(sala_id, usuario_id):
    """Adiciona um usuário à lista de banidos de uma sala."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            cursor = conexao.cursor()
            cursor.execute(
                "INSERT OR IGNORE INTO banidos_sala (sala_id, usuario_id) VALUES (?, ?)",
                (sala_id, usuario_id)
            )
            conexao.commit()
            return True
...
def verificar_banido(sala_id, usuario_id):
    """Retorna True se o usuário estiver banido da sala."""
    try:
        with sqlite3.connect(NOME_DB) as conexao:
            cursor = conexao.cursor()
            cursor.execute(
                "SELECT id FROM banidos_sala WHERE sala_id = ? AND usuario_id = ?",
                (sala_id, usuario_id)
            )
            return cursor.fetchone() is not None
    except Exception as e:
        print(f"Erro ao verificar banido: {e}")
        return False
